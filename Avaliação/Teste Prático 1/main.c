...
void* slaveWork(void* a) {
	...
	char env_buffer_chars[CHARBUFFSIZE];
	...
	/*Diz que esta pronta*/
	snprintf(env_buffer_chars, CHARBUFFSIZE, "Tarefa %d pronta!\n", myid);
	enviarMensagem(myid, 0, env_buffer_chars, sizeof(char)*CHARBUFFSIZE);
...
int main(int argc, char** argv) {
	...
	char buffer_chars[CHARBUFFSIZE];
	...
	/*Receber mensagem a dizer que esta pronta*/
	for (i = 1; i < trab+1; i++) {
		receberMensagem(i, 0, buffer_chars, sizeof(char)*CHARBUFFSIZE);
		printf("%s", buffer_chars);
	}
