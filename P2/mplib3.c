...
pthread_cond_t		*waitezonos_for_free_space;
pthread_cond_t		*waitezonos_for_messages;
...
int inicializarMPlib(int capacidade_de_cada_canal, int ntasks) {
	...
	if (waitezonos_for_free_space == NULL) {
		fprintf(stderr, "\nErro ao alocar memória para MPlib\n");
		return -1;
	}
	if (waitezonos_for_messages == NULL) {
		fprintf(stderr, "\nErro ao alocar memória para MPlib\n");
		return -1;
	}
	...
	for (i = 0; i < ntasks; i++) {
		for (j = 0; j < ntasks; j++) {
			...
			if (pthread_mutex_init(&mutexes[i*number_of_tasks + j], NULL) != 0) {
				fprintf(stderr, "\nErro ao inicializar mutex\n");
				return -1;
			}

			if (pthread_cond_init(&waitezonos_for_free_space[i*number_of_tasks + j], NULL) != 0) {
				fprintf(stderr, "\nErro ao inicializar variável de condição\n");
				return -1;
			}

			if (pthread_cond_init(&waitezonos_for_messages[i*number_of_tasks + j], NULL) != 0){
				fprintf(stderr, "\nErro ao inicializar variável de condição\n");
				return -1;
			}
...
void libertarMPlib() {
	...
	for (i = 0; i < number_of_tasks; i++) {
		for (j = 0; j < number_of_tasks; j++) {
			...
			if (pthread_cond_destroy(&waitezonos_for_free_space[i*number_of_tasks + j]) != 0) {
				fprintf(stderr, "\nErro ao destruir variável de condição\n");
				exit(EXIT_FAILURE);
			}

			if (pthread_cond_destroy(&waitezonos_for_messages[i*number_of_tasks + j]) != 0) {
				fprintf(stderr, "\nErro ao destruir variável de condição\n");
				exit(EXIT_FAILURE);
			}
	...
	free(waitezonos_for_messages);
	free(waitezonos_for_free_space);
	free(mutexes);
	free(channel_array);
}
...
int receberMensagem(int tarefaOrig, int tarefaDest, void *buffer, int tamanho) {
	...
	while (!mess) {
		if (pthread_cond_wait(&waitezonos_for_messages[tarefaDest*number_of_tasks + tarefaOrig], &mutexes[tarefaDest*number_of_tasks + tarefaOrig]) != 0) {
	...
	if (pthread_cond_broadcast(&waitezonos_for_free_space[tarefaDest*number_of_tasks + tarefaOrig]) != 0) {
...
int enviarMensagem(int tarefaOrig, int tarefaDest, void *msg, int tamanho) {
	...
	if (channel_capacity > 0) {
		while (leQueSize(channel->message_list) >= channel_capacity) {
			if(pthread_cond_wait(&waitezonos_for_free_space[tarefaDest*number_of_tasks + tarefaOrig], &mutexes[tarefaDest*number_of_tasks + tarefaOrig]) != 0) {
				fprintf(stderr, "\nErro ao esperar pela variável de condição\n");
				return -1;
			}
		}
	...
	pthread_cond_broadcast(&waitezonos_for_messages[tarefaDest*number_of_tasks + tarefaOrig]);
