...
pthread_cond_t barreira, barreira2;
...
int threads_on_wait = 0, chegou_a_ultima = 0, iteracao = 0, contaSai=0;
int go_maxD = 1;
double max_min = 0;
...
/*void esperarBarreira(int trabs) {
	...
	if (threads_on_wait == trabs) {
		...
		while (autorizacoes <= threads_on_wait) {
			pthread_cond_wait(&barreira, &mutex);
		}
	}
	autorizacoes--;
...
void* slaveWork(void* a) {
	...
	for (i = 0; i < iteracoes && go_maxD; i++) {
		...
		threads_on_wait++;
		if (threads_on_wait == trabs){
			contaSai = trabs - 1; //qnts vao saindo
			chegou_a_ultima = 1;
			threads_on_wait = 0;
			pthread_cond_broadcast(&barreira);
		} else {

			while (contaSai) pthread_cond_wait(&barreira2, &mutex);
			while (!chegou_a_ultima) {
				pthread_cond_wait(&barreira, &mutex);
			}
			contaSai--;
		}
		if (contaSai == 0) {
			chegou_a_ultima = 0;
			pthread_cond_broadcast(&barreira2);
		}
		pthread_mutex_unlock(&mutex);
		onwait = 1
		consai = 1
		...
		*/
		...
		printf("aqui A ------- %d\n", myid);
		printf("onwait%d, autor%d\n", threads_on_wait, iteracao);
		pthread_mutex_lock(&mutex);
		printf("aqui B ------- %d\n", myid);
		printf("onwait%d, autor%d\n", threads_on_wait, iteracao);
